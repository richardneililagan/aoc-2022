...
module Year2022
  class Grid
    attr_reader :visible_map
    attr_reader :scenic_scores

    def initialize(input_area)
      area, visible_map, scenic_scores = parse_input(input_area)
    ...
    def parse_input(input_area)
      area = []
      visible_map = []
      scenic_scores = []

      input_area.each do |row|
    ...
    def parse_row_visibility(j, from, to)
      current_max_height = -1

      range = (from < to) ? (from..to) : from.downto(to)

      range.each do |i|
        current_tree_height = @area[j][i]
    ...
    def parse_column_visibility(i, from, to)
      current_max_height = -1
